...
class COMALearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # self.icm_learner = ICMLearner(mac, scheme, logger, args)
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int,
                  mac, optimiser, params,intrinsic_rewards,
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        max_t = batch.max_seq_length
        rewards = batch["reward"][:, :-1]
        print("TEST1 shape:",len(batch["reward"]),len(batch["reward"][0]))
        print("TEST2:",len(rewards),len(rewards[0]))
        ...
        avail_actions = batch["avail_actions"][:, :-1]
        # ------------------------------------------------------------------------------------------
        print("CHECK avail_actions:", len(avail_actions),len(avail_actions[0]))
        ...
        print("mac_out shape:", len(mac_out))

        # Calculated baseline
        q_vals = q_vals.reshape(-1, self.n_actions)
        pi = mac_out.view(-1, self.n_actions)
        baseline = (pi * q_vals).sum(-1).detach()
        print("Q_vals shape:", len(q_vals))
