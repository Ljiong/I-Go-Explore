...
from .icm_learner import ICMLearner

class MADDPGLearner:
    def __init__(self, mac, imac, scheme, logger, args):
        self.args = args
    ...
    def train(self, batch: EpisodeBatch, reward_learner, t_env: int, episode_num: int):
        ...
        # print("maddpg learner, rewards shape:", rewards.shape)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            # print("inMADDPG_LOG")
            self.logger.log_stat("critic_loss", loss.cpu().item(), t_env)
            self.logger.log_stat("critic_grad_norm", critic_grad_norm.cpu().item(), t_env)
            self.logger.log_stat("agent_grad_norm", agent_grad_norm.cpu().item(), t_env)
            mask_elems = mask.sum().item()
            self.logger.log_stat("td_error_abs", masked_td_error.abs().sum().cpu().item() / mask_elems, t_env)
            self.logger.log_stat("q_taken_mean", (q_taken).sum().cpu().item() / mask_elems, t_env)
            self.logger.log_stat("target_mean", targets.sum().cpu().item()/ mask_elems, t_env)
            self.logger.log_stat("pg_loss", pg_loss.cpu().item(), t_env)
