...
class _GymmaWrapper(MultiAgentEnv):
    def __init__(self, time_limit, pretrained_wrapper, seed, key="rware:rware-small-4ag-v1", max_steps=30000, max_inactivity_steps=30000, **kwargs):
        # self.original_env = gym.make(f"{key}", f"{max_steps}", f"{max_inactivity_steps}", **kwargs)
        self.original_env = gym.make(key, max_steps=30000,max_inactivity_steps=30000)
