...
data = pd.read_csv('E:\\study\\Thesis\\epymarl\\results\\rware\\FINAL_COMA_SMALL_500.csv')
...
if __name__ == '__main__':
    ...
    plt.fill_between(np.arange(0,500), data['coma_ige'] - 0.02, data['coma_ige'] + 0.02, color="honeydew", alpha=0.95)
    plt.fill_between(np.arange(0,500), data['coma'] - 0.02, data['coma'] + 0.02, color="lightyellow", alpha=0.95)
    plt.fill_between(np.arange(0,500), data['coma_icm'] - 0.02, data['coma_icm'] + 0.02, color="aliceblue", alpha=0.95)
    ...
    plt.savefig("E:/study/Thesis/epymarl/results/rware/coma-SMALL-4g-500.png",dpi=500,bbox_inches = 'tight')
    plt.show()