...
data = pd.read_csv('E:\\study\\Thesis\\epymarl\\results\\rware\\FINAL_maddpg_SMALL.csv')
...
if __name__ == '__main__':
    ...
    plt.fill_between(np.arange(0,25), data['maddpg_ige'] - 0.039, data['maddpg_ige'] + 0.039, color="honeydew", alpha=0.9)
    plt.fill_between(np.arange(0,25), data['maddpg'] - 0.034, data['maddpg'] + 0.034, color="lightyellow", alpha=0.9)
    plt.fill_between(np.arange(0,25), data['maddpg_icm'] - 0.038, data['maddpg_icm'] + 0.038, color="aliceblue", alpha=0.9)
    ...
    plt.savefig("E:/study/Thesis/epymarl/results/rware/MADDPG-SMALL-4g.png",dpi=500,bbox_inches = 'tight')
    plt.show()